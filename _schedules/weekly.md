---
timeline:
  - "9:00 AM"
  - "10:00 AM"
  - "10:30 AM"
  - "11:00 AM"
  - "11:30 AM"
  - "12:00 PM"
  - "1:00 PM"
  - "2:00 PM"
  - "3:00 PM"
  - "3:30 PM"
  - "4:00 PM"
  
schedule:
  - name: Monday
    events:
      - name: section
        start: 9:00 AM
        end: 10:00 AM
        location: Introductions
      - name: Section
        start: 11:00 AM
        end: 12:00 PM
        location: Background and History
      - name: Section
        start: 12:00 PM
        end: 1:00 PM
        location: Introduction to plotting with RStudio
      - name: Section
        start: 1:00 PM
        end: 2:00 PM
        location: Transcriptom analysis activity
      - name: Section
        start: 2:00 PM
        end: 3:00 PM
        location: Proteomics analysis activity
      - name: Section
        start: 3:00 PM
        end: 4:00 PM
        location: Final comments and questions
  - name: Tuesday
      events:
      - name: section
        start: 9:00 AM
        end: 10:00 AM
        location: Activity Recap
      - name: Section
        start: 11:00 AM
        end: 12:00 PM
        location: Introduction to different PI3 systemp
      - name: Section
        start: 12:00 PM
        end: 1:00 PM
        location: Discussion of experimental design
      - name: Section
        start: 1:00 PM
        end: 2:00 PM
        location: Flow Cytometry, microscopy and inducing polyploidy
  - name: Wednesday
      events:
      - name: section
        start: 9:00 AM
        end: 10:00 AM
        location: Tean science
      - name: Section
        start: 11:00 AM
        end: 12:00 PM
        location: Cancer
      - name: Section
        start: 12:00 PM
        end: 1:00 PM
        location: Agriculture
      - name: Section
        start: 1:00 PM
        end: 2:00 PM
        location: Careers Panel
      - name: Section
        start: 2:00 PM
        end: 3:00 PM
        location: Assessment
---
